    ...
    if (state.phase === 'done' && !sessionCompleteFired.current) {
      ...
      if (typeof window !== 'undefined' && typeof window.fbq === 'function') {
        window.fbq('trackCustom', 'SessionComplete', {
          test_scope: state.testScope ?? 'full',
          session_id: state.sessionPublicId,
        });
      }
    } else if (state.phase === 'idle') {
  ...
  return (
    <DemoGuardProvider value={contextValue}>
      <div className="app-shell" style={shellStyle}>
        ...
        {state.phase === 'done' && (
          <DoneScreen response={state.response} cognitiveSignals={state.cognitiveSignals} testScope={state.testScope} motionSignal={state.signals.motion} orientationSignal={state.signals.orientation} onReset={handleReset} />
        )}
